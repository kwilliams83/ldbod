...
#' @title Local Density-Based Outlier Detection using random subsampling with flexible nearest neighbor search.
#' @param X An n x p data matrix to compute outlier scores
#' @param k Neighborhood size, k must be less than n_sub
...
#'
#' @details Computes the local density-based outlier scores for input data X referencing a random subsample of the input data X. The subsampled
...
#' Methods LDF and RKOF uses guassian kernels, and method LDPF uses multivarite t distribution. Outlier scores returned are non-negative
#' except for lpde adn lpdr which are log scaled densities (natural log).
#'
  ...
  for(kk in k){
    ...
    if(sum(method%in%c('lof','ldf'))>0){
      ...
      if('ldf'%in%method){
        ...
        lde <- sapply(1:n,function(id)mean(1/((2*pi)^(p/2))*1/(h*dist_k[sub_sample_ids[knn_ids[id,1:kk]]])^p*exp(-(.5*reach_dist_matrix_test[id,]^2)/(h*dist_k[sub_sample_ids[knn_ids[id,1:kk]]])^2))+1e-200)
    ...
    if('lpdf'%in%method){
      ...
      for(t in 1:tmax){
        ...
        # compute updated weight function, R
